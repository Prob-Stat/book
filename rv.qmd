...
::: {.callout-note}
本当の確率論の言葉で述べると任意のボレル関数(開集合(ボレル集合としても等価)の逆像がボレル集合) $f$ について $f\circ X$ は確率変数である。
:::
...
$$
f_X(x) \coloneqq \Pr(X=x) \forall x\in\mathbb{R}
$$

という関数を用いて $\Pr(X\in A) = \sum_{x\in A} f_X(x)$ と表せる。
