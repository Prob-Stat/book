...
コインを2回独立に投げる場合の確率空間 $(\Omega,\,P)$ を以下で定義する。
...
X_2(\mathrm{TT}) &= 0
\end{align*}
とすると、$k\in\{1,2\}$ について、$X_k$ は $k$ 回目のコイン投げが表だったときに1、裏だったときに0になる。
また、$X=X_1+X_2$ は表が出た回数を表す確率変数となる。
表が一回以上出る確率は以下のように表せる。
\begin{align*}
...
確率変数 $X$ について**累積分布関数** $F_X\colon\mathbb{R}\to\mathbb{R}_{\ge 0}$ は以下で定義される。
...
::: {#lem-pdf-inc}
$J\subseteq\mathbb{R}$ を有界とは限らない区間とし、$g\colon J\to\mathbb{R}$ を $J$ の内点で**微分可能**で $g'(x)>0$ と
する。
