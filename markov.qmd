...
グラフの連結成分 $C\subseteq V$ について、列ベクトル $\mathbf{1}_C$ を成分のインデックスが $C$ に含まれるとき1、そうでないとき 0 と定義する。
このとき、$T\mathbf{1}_C=\mathbf{1}_C$ であるので、固有値1の右固有ベクトルとなる。
各連結成分 $C$ について $\mathbf{1}_C$ は線形独立であるので、「固有値1の重複度 $\ge$ グラフの連結成分の個数」が示された。
...
\begin{align*}
v&\in\mathrm{span}(\mathbf{1}_{C_1},\dotsc,\mathbf{1}_{C_k})
\end{align*}
である。
...
グラフの二部連結成分 $L\cup R\subseteq V$ について、列ベクトル $\mathbf{1}_{L,R}$ を成分のインデックスが $L$ に含まれるとき $+1$、$R$ に含まれるとき $-1$、それ以外のとき 0 と定義する。
このとき、$T\mathbf{1}_{L,R}=-\mathbf{1}_{L,R}$ であるので、固有値$-1$の右固有ベクトルとなる。
各二部連結成分 $L\cup R$ について $\mathbf{1}_{L,R}$ は線形独立であるので、「固有値 $-1$ の重複度 $\ge$ グラフの二部連結成分の個数」が示された。
...
\begin{align*}
v&\in\mathrm{span}(\mathbf{1}_{L_1,R_1},\dotsc,\mathbf{1}_{L_k,R_k})
\end{align*}
である。
