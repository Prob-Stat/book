...
\end{align*}
と定義される。
ここで、右辺の $\|\cdot\|_2$ は **$L_2$ ノルム**を表す。ここで、
\begin{align*}
\|A\|_2&= \max_{x\in\mathbb{C}^m \setminus\{0\}} \sqrt{\frac{\|Ax\|^2_2}{\|x\|^2_2}}\\
&=\sqrt{\max_{x\in\mathbb{C}^m \setminus\{0\}} \frac{x^*A^*Ax}{\|x\|^2_2}}\\
&=\sqrt{\max_{x\in\mathbb{C}^m\colon \|x\|_2=1} x^*A^*Ax}\\
&=\sqrt{\lambda_\max(A^*A)}\\
&=\rho_\max(A)
\end{align*}
である。$\lambda_\max$ と $\rho_\max$ は最大固有値と最大特異値を表す。
...
このとき、$A^*A=UD^*DU^*$ なので、$\|A\|_2=\rho(A)$ である。

しかし一般的には(対角化可能であっても)スペクトル半径とスペクトルノルムは異なる。
例えば、任意に選んだ $a\in\mathbb{C}$ について
\begin{align*}
A&=\begin{bmatrix}
1&a\\
0&0
...
\end{bmatrix}
\end{align*}
より、$A^*A$ の固有値は 0 と $1+|a|^2$ である。
よって、スペクトルノルムは $\|A\|_2=\sqrt{1+|a|^2}$ である。
これは $a\ne 0$ のとき、これはスペクトル半径 $\rho(A)$ より真に大きい。
また、この $A$ は二つの異なる固有値を持つので対角化可能である。
:::
...
:::{#lem-graph-eigenb}
グラフ上のランダムウォークの遷移行列 $T$ に含まれる固有値 $-1$ の個数はグラフの**二部連結成分の個数**である。
:::
:::{.proof}
まず、「固有値  $-1$ の個数 $\ge$ グラフの二部連結成分の個数」を証明する。
...
各二部連結成分 $L\cup R$ について $\mathrm{1}_{L,R}$ は線形独立であるので、「固有値 $-1$ の個数 $\ge$ グラフの二部連結成分の個数」が示された。

次に、「固有値 $-1$ の個数 $\le$ グラフの二部連結成分の個数」を証明する。
...
\end{align*}
である。
以上のことから「固有値 $-1$ の個数 $\le$ グラフの二部連結成分の個数」が示された。
:::
