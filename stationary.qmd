...
マルコフ連鎖が既約であることから、任意の $j\in S$ について $i\le j$ であり、$v = v_i \mathbf{1}$ である。
...
::: {#thm-irred}
既約なマルコフ連鎖は唯一の定常分布 $\mu\in\mathbb{R}_{>0}^S$ を持つ。
:::
:::{.proof}
...
次に、$\mathrm{adj}(\Lambda)\Lambda=O$ より、$\mathrm{adj}(\Lambda)$ の行 $\mu\in\mathbb{R}^S$ は $\mu\Lambda=0$ を満たす。
...
:::{#lem-stat-trans}
マルコフ連鎖の定常状態を $\mu\in\mathbb{R}^S$ とすると、再帰的でない状態 $s\in S$ について $\mu(s)=0$ である。
:::
:::{.proof}
定常分布 $\mu\in\mathbb{R}^S$ が再帰的でない状態 $s\in S$ について$\mu(s)>0$ であると仮定する。
